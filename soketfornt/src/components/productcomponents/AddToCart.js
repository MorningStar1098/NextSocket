...
import { cartadd } from "../redux/actions/Actions";
...
const AddToCart = () => {
  ...
  const cart=useSelector((state)=>state?.carts?.addtocartslice)  
 
  useEffect(() => {
    dispatch(cartadd())
  }, [deleteData, cartdata,token,dispatch]);

  const deleteProduct = (productId) => {
    axios
      .delete(`${LIVE_URL}/api/products/delete/${productId}`, {
  ...
  const updateQuantity = (productId, quantity) => {
    axios
      .put(
        `${LIVE_URL}/api/products/updatequantities/${productId}`,
        { quantity: quantity },
        {
          headers: { Authorization: ` ${token}` },
        }
