...
function ResetPassword() {
    ...
    function handleContinue(token) {
        ...
        axios.get(`${LIVE_URL}/api/v1/get/user/data/${token}`, {
            ...
            const res = response && response.data && response.data.data !== undefined ? response.data.data : null
            setUserData(res)
            setTimeout(()=>{
                setLoadingErr(false)
              },500)
            const dateTime = res?.creat_at
            if (dateTime !== undefined) {
                const currentdate = new Date()
                const date = new Date(dateTime);
                date.setHours(date.getHours() + 72);
                const expiryTime = Date.parse(date)
                                    ...
                                    <form>
                                        <div className='loginuser-name confirm-pass-inputbox'>
                                            ...
                                            <button className="view-icon" onClick={toggleoldPassword}>
                                                {oldpasswordType == "password" ? <AiOutlineEye /> : <AiOutlineEyeInvisible />}</button>
                                            <input type={oldpasswordType} id='confirm-pass' onChange={handleEmail} />
                                            {errorMessage === '' ? null :
                                                <span className={errorMessage == "Strong Password" ? "password-strong" : "password-not-strong"}>{errorMessage}</span>}
                                        ...
                                        <div className='button-container reset-pass-continuebtn'>
                                            {passwordState == true ? <span className='password-not-strong'>This password is too weak .Try with new password</span> : null}
                                            <button className="loaderImageS-socialmedia" onClick={handleUpdPass} >{loading ? <LoadingIcons.Bars /> : "Submit"}</button>
                                        </div>
                                    </form>
                                </>}
                        </>}
